...
- **斗地主同步**：`/api/game/sync` 共用身份和房间校验，先推进超时/人机，再按版本返回变更后的本人视图及独立聊天游标。无变化返回 `room: null`；成功操作采用 POST 回执，冲突或结果不明只刷新、不自动重试。前台活跃阶段最多每秒一次，等待/局间每三秒一次，后台暂停。倒计时与校时独立于牌桌快照。
