...
function Sound({room,userId,target}:{room:Room;userId:string;target:HTMLElement|null}){const seconds=useMahjongSeconds(room);return <TableSound room={room} userId={userId} seconds={seconds} controlsTarget={target}/>}
...
export function MahjongRoom({room,userId,busy,selected,onSelect,connected,bubbles,onSnapshot,onAction,onBack,onLeave}:{room:Room;userId:string;busy:boolean;selected:number|null;onSelect:(n:number|null)=>void;connected:boolean;bubbles:Record<string,ChatBubble>;onSnapshot:(snapshot:ChatSnapshot)=>void;onAction:Action;onBack:()=>void;onLeave:(action:'leave'|'end_table'|'end_practice')=>void}){
 ...
 const [menuOpen,setMenuOpen]=useState(false),[soundTarget,setSoundTarget]=useState<HTMLDivElement|null>(null);
 const highlightType=g.phase==='playing'&&selected!==null&&me.hand.includes(selected)?Math.floor(selected/4):null;
 ...
 const ownRestingCount=Math.max(0,13-me.melds.length*3),ownDraw=ownTiles.length>ownRestingCount?ownTiles.at(-1):undefined;
...
{g.phase!=='closed'&&!me.departed&&<><Button variant="outline" disabled={busy||me.leaving} onClick={()=>onLeave('leave')}><LogOut size={17}/>{me.leaving?'结算后离开':solo?'托管至结算后离开':'离开房间'}</Button>{g.host===userId&&g.session&&<Button variant="ghost" disabled={busy||!solo&&(me.leaving||!['waiting','finished'].includes(g.phase))} onClick={()=>onLeave(solo?'end_practice':'end_table')}><DoorOpen size={17}/>{solo?'立即结束测试':'结束整桌'}</Button>}</>}</div></SheetContent></Sheet>
 ...
 <MahjongSurface g={g} seat={seat} clock={room} bubbles={bubbles} botControls={botControls} highlightType={highlightType}>
 ...
 <div className="mj-right-tools"><RoomChat compact key={room.code} code={room.code} userId={userId} onSnapshot={onSnapshot} closed={g.phase==='closed'||me.departed}/><ExpandMahjong g={g} seat={seat} highlightType={highlightType}/></div>
