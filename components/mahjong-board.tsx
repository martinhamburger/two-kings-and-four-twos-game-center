...
function River({s,g,expanded=false,highlightType=null}:{s:Seat;g:MahjongView;expanded?:boolean;highlightType?:number|null}){
 ...
 <div className="mj-discards">{s.river.map(t=><span key={t} className={g.lastDiscard?.seat===g.seats.findIndex(player=>player.id===s.id)&&g.lastDiscard.tile===t?'latest-tile':''}><MahjongTile tile={t} small wildcard={g.wildcard} highlightType={highlightType}/></span>)}</div>
...
export const MahjongSurface=memo(function MahjongSurface({g,seat,clock,children,botControls,bubbles={},expanded=false,highlightType=null}:{g:MahjongView;seat:number;clock?:MahjongClockSource;children?:ReactNode;botControls?:BotSeatControls;bubbles?:Record<string,ChatBubble>;expanded?:boolean;highlightType?:number|null}){
 ...
 {positions.map((position,relative)=>{const index=(seat+relative)%4,s=g.seats[index];return <div className={`mj-seat-layer zone-${position}`} key={position}><MahjongPortrait g={g} index={index} position={position} own={relative===0} botControls={botControls} bubble={s?bubbles[s.id]:undefined}/>{s&&relative!==0&&<PlayerRack s={s} g={g} position={position} highlightType={highlightType}/>}<div className={`mj-river-position river-${position}`}>{s&&<River key={g.round+':'+s.id} s={s} g={g} expanded={expanded} highlightType={highlightType}/>}</div></div>})}
